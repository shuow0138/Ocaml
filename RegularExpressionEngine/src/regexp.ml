...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
    ...
    let st2 = fresh() in 
  {
    ...
    qs = [st1 ; st2];
    fs = [st2];
    delta = [(st1, Some c, st2)];
  }
  | Union (r1, r2) ->
    let r1' = regexp_to_nfa r1 in 
    let r2' = regexp_to_nfa r2 in 
    let st1 = fresh() in 
    let st2 = fresh() in 
    {
      sigma = Sets.union (r1'.sigma) (r2'.sigma);
      q0 = st1;
      qs = Sets.union r1'.qs (Sets.union r2'.qs [st1;st2]);
      fs = [st2];
      delta = Sets.union r1'.delta ( Sets.union r2'.delta [((hd r2'.fs), None, st2) ; (st1, None, r2'.q0) ; ((hd r1'.fs), None, st2) ; (st1, None, r1'.q0)]);
    }
  | Concat (r1,r2) -> 
    let r1' = regexp_to_nfa r1 in 
    let r2' = regexp_to_nfa r2 in 
    {
      sigma = Sets.union r1'.sigma r2'.sigma;
      q0 = r1'.q0;  
      qs = Sets.union r1'.qs r2'.qs;
      fs = r2'.fs;
      delta =  Sets.union (Sets.union r1'.delta r2'.delta)  [((hd r1'.fs), None, r2'.q0)];
    }
  | Star (r) ->
    let r' = regexp_to_nfa r in
    let st1 = fresh() in
    let st2 = fresh() in
     {
      sigma = r'.sigma; 
      q0 = st1;  
      qs = Sets.union r'.qs [st1;st2];
      fs = [st2];
      delta =  Sets.union r'.delta [((hd r'.fs), None, st2) ; (st1, None, r'.q0) ; (st1, None, st2) ; (st2, None, st1)];
     }
